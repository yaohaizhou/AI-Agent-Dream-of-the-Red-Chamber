...
async def test_chapter_planner():
    """测试ChapterPlannerAgent的基本功能"""
    
    print("=" * 60)
    print("ChapterPlannerAgent 测试")
    print("=" * 60)
    ...
    if result.success:
        ...
        for phase_name, phase_info in narrative_phases.items():
            ...
            print(f"    描述: {phase_info.get('description', '')[:40]}...")
...
if __name__ == "__main__":
    ...
    # 询问是否进行完整测试（需要API）
    print("\n" + "=" * 60)
    user_input = input("\n是否进行完整测试（需要调用GPT-5 API）？[y/N]: ")
    
    if user_input.lower() == 'y':
        asyncio.run(test_chapter_planner())
    else:
        print("\n跳过完整测试。")
        print("提示: 运行 'python tests/test_chapter_planner.py' 并输入 'y' 进行完整测试")
