...
from ...utils.base_scorer import ScoreConfig, StyleEvaluator, StructureEvaluator, safe_score
...
class QualityCheckerAgent(BaseAgent):
    ...
    def _fallback_style_evaluation(self, content: str) -> float:
        """备用风格评估方法 - 使用基类StyleEvaluator"""
        return self.style_evaluator.evaluate(content)
    ...
    def _check_personality_match(self, content: str, char_name: str, personality_traits: str) -> float:
        ...
        返回0-1之间的匹配度
        """
        # 定义核心关键词映射 (从长描述提取关键词) - V2扩展版
        keyword_mapping = {
            # 宝玉
            "纯真": ["真", "纯", "痴", "傻", "呆"],
            "善良": ["善", "怜", "疼", "爱", "心", "软"],
            "叛逆": ["叛逆", "不听", "不愿", "不想", "偏", "反"],
            "封建": ["礼教", "规矩", "功名", "仕途", "经济", "读书"],
            "细腻": ["细", "心", "想", "思", "念", "愁"],
            "尊重": ["尊重", "疼", "爱", "怜", "惜", "护"],
            # 黛玉
            "聪慧": ["诗", "词", "才", "慧", "灵"],
        ...
        for trait in personality_traits.split():
            ...
            # 策略1: 尝试完整匹配
            keywords = keyword_mapping.get(trait)
            if keywords:
                for kw in keywords:
            ...
            if not found:
                for char in trait:
                    if char in content_lower and char in "真善良愁悲泪叹诗词才慧贤稳重聪慧敏纯叛逆封细腻尊重端蕙圆精干权慈祥威":
                        matches += 1
                        found = True
                        break
            
            # 策略4: 最后尝试简单包含
            if not found and trait in content_lower:
                matches += 1
    ...
    def _check_behavior_consistency(self, content: str, char_name: str, char_info: Dict[str, str]) -> float:
        ...
        behavior_keywords = {
            # 宝玉
            "关心": ["关心", "疼", "爱", "问", "瞧", "看", "望", "探"],
            "女性": ["妹妹", "姐姐", "女儿", "丫鬟", "姑娘", "小姐"],
            "逃避": ["逃", "躲", "避", "怕", "烦", "厌", "恶"],
            "仕途": ["仕途", "功名", "读书", "做官", "老爷", "经济"],
            "诗词": ["诗", "词", "曲", "赋", "联", "句", "吟"],
            # 黛玉
            "写诗": ["诗", "词", "写", "作", "吟", "咏", "赋"],
            "体弱": ["病", "弱", "咳", "喘", "药", "痛", "虚"],
